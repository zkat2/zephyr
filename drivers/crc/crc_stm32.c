...
static int crc_lock(const struct device *dev, k_timeout_t timeout)
{
...
static int crc_unlock(const struct device *dev)
{
	struct crc_stm32_data *data = (struct crc_stm32_data *)dev->data;
...
static int crc_stm32_begin(const struct device *dev, struct crc_ctx *ctx)
{
	...
	/* Attempt to take mutex lock */
	const struct crc_stm32_data *data = dev->data;
	int mtx = crc_lock(dev, data->timeout);
...
static int crc_stm32_finish(const struct device *dev, struct crc_ctx *ctx, const void *buffer,
			    size_t bufsize)
{
...
static int crc_stm32_init(const struct device *dev)
{
	...
	if (CONFIG_CRC_TIMEOUT_MS == 0) {
		data->timeout = K_NO_WAIT;
	} else if (CONFIG_CRC_TIMEOUT_MS > 10000) {
		data->timeout = K_FOREVER;
	} else {
		data->timeout = K_MSEC(CONFIG_CRC_TIMEOUT_MS);
	}
