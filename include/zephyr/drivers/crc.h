 ...
 */
struct crc_ctx {
	enum crc_type type;		/**< Type of CRC algorithm */
...
typedef int (*crc_api_finish)(const struct device *dev, struct crc_ctx *ctx, const void *buffer,
			      size_t bufsize);
 ...
 * @retval 0 if successful, negative errno code on failure
 */
__syscall int crc_finish(const struct device *dev, struct crc_ctx *ctx, const void *buffer,
			 size_t bufsize);
				    ...
				    const void *buffer, size_t bufsize)
{
	...
	return api->crc_finish(dev, ctx, buffer, bufsize);
}
 ...
 * @retval 0 if successful, negative errno code on failure
 */
static inline int crc_verify(const struct device *dev, struct crc_ctx *ctx, crc_result_t expected)
{
